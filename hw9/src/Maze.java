...
public class Maze {
  ...
  public Maze(int horizontalSize, int verticalSize) {
    ...
    if( horiz*vert > 1){
      String[] totalWalls = new String[2 * horiz * vert - horiz - vert];   
    if(horiz>1&&vert>1){
    	int k =0;
    	  for (j = 0; j < vert - 1; j++){ 
    		  for (i = 0; i < horiz; i++){ 
            	  totalWalls[k] ='h'+ Integer.toString(i) + ','+Integer.toString(j);
            	  k++;
    		  }           	  
    	  }
          for (i = 0; i < horiz - 1; i++){
        	  for (j = 0; j < vert; j++){
        		  totalWalls[k] = 'v'+ Integer.toString(i) + ','+ Integer.toString(j);  
        		  k++;
        	  }   	  
          }      	  
    ...
    int w = totalWalls.length;
    while(w!=1){
      int index = 1;
      int des = randInt(w);
      String temp = totalWalls[des];
      totalWalls[des] = totalWalls[totalWalls.length - index];
      totalWalls[totalWalls.length-index] = temp;
      w--;
      
    }
    
    for(i = 0; i<totalWalls.length;i++){
      char type = totalWalls[i].charAt(0);
      int div = totalWalls[i].indexOf(',');
      int x = Integer.parseInt(totalWalls[i].substring(1, div));
      int y = Integer.parseInt(totalWalls[i].substring(div + 1));
      int neighbor1,neighbor2;
      if (type == 'h'){
        neighbor1 = x + y * vert;
        neighbor2 = x + (y + 1) * vert;
      }else{
        neighbor1 = x + y * vert;
        neighbor2 = x + 1 + y*vert; 
      }
      int root1 = maze.find(neighbor1);
      int root2 = maze.find(neighbor2);
      if (root1 != root2){
    	  maze.union(root1,root2);
        if(type == 'h')
          hWalls[x][y] = false;
        else
          vWalls[x][y] = false;
