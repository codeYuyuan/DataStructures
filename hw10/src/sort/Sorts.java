...
public class Sorts {
  ...
  public static int[] countingSort(int[] keys, int whichDigit) {
    int[] keyForSort = new int[keys.length];
    int shift = 4 * whichDigit;
    for(int i = 0; i< keys.length;i++){
      keyForSort[i] = keys[i] >> shift & 15;
    }
    int[] counts = new int[16];
    for (int i =0;i<keyForSort.length ; i++) {
      counts[keyForSort[i]]++;      
    }
    int total = 0;
    ...
    for (int i =0;i<keys.length ; i++) {
      sorted[counts[keyForSort[i]]] = keys[i];
      counts[keyForSort[i]]++;     
    }
